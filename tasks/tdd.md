...
| ---- | ----- | --------- |
| `test/unit/test_base_field_contract.py` | Unit | `BaseField` shared contract: accepted optional attributes, invalid labels/descriptions/policies/extra keys. |
...
| `test/unit/test_factory_and_context.py` | Unit | Builtin kind order, builder callability, and `FieldContext` metadata preservation. |
| `test/unit/test_field_models.py` | Unit | Builtin Pydantic model success cases and constraint failures for boolean/category/date/number/series/text. |
| `test/unit/test_inference_errors.py` | Unit | Known inference exceptions: empty DataFrame, missing overrides, invalid builder output, missing/unknown kind, duplicate kind, validation failures, root exception inheritance. |
| `test/unit/test_kind_api.py` | Unit | `kind()` API: kind-name extraction from model and non-`BaseField` rejection. |
| `test/integration/test_infer_schema_workflows.py` | Integration | End-to-end workflows with all builtins, overrides, custom builder, and custom strict kind. |
...
| `test/load/test_infer_schema_load.py` | Load | Inference correctness under many rows/columns and large categorical option sets. |
